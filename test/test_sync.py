...
class TestSync(unittest.TestCase):
    ...
    def workspace_create_random(self, name, size):
        ...
        f = open(self.local_workspace + '/' + name, 'w')
    ...
    def local_write_file(self, name, contents):
        self.write_file(self.local + '/' + name, contents)

    def workspace_write_file(self, name, contents):
        self.write_file(self.local_workspace + '/' + name, contents)
    ...
    def create_tree(self, file_list):
        ...
        for file in file_list:
            ...
            for component in path[:-1]:
                ...
                try:
                    os.mkdir(self.local_workspace + '/' + components)
                except OSError:
                    # directory already exists
                    pass
                components += '/'
            f = open(self.local_workspace + '/' + file, 'w')
            f.write(file)
            f.close()

    def test_clone_tree(self):
        """Create a tree of files and directories and check if it clones."""
        self.create_tree(['red.file', 'green.file', 'blue.file',
    ...
    def test_push_without_clone(self):
        os.mkdir(self.local)
        self.assertRaises(
            pithossync.NoCloneError,
            self.syncer.push,
            self.local
        )
